...
class MyRobotDriver:
    def init(self, webots_node, properties):
        ...
        self.wait_duration = 2.0  # Wait for 2 seconds
        self.push_start_time = None
        self.push_duration = 3.0  # Push for 3 seconds before waiting
        self.reposition_distance = 0.1  # Distance behind the ball

        rclpy.init(args=None)
        self.__node = rclpy.create_node("my_robot_main")
        self.__node.create_subscription(Twist, "cmd_vel", self.__cmd_vel_callback, qos_profile=rclpy.qos.qos_profile_sensor_data)
        self.movementPublisher = self.__node.create_publisher(Twist, "cmd_vel", qos_profile=rclpy.qos.qos_profile_sensor_data)
        ...
        self.__node.get_logger().info(f"The left wall is at {self.leftWallNode.getPosition()}")
        self.__node.get_logger().info(f"The right wall is at {self.rightWallNode.getPosition()}")
    ...
    def dist(self) -> float:
        ...
        return math.sqrt((ballPos[0] - puckPos[0])**2 + (ballPos[1] - puckPos[1])**2)
    ...
    def movetoPoint(self, xPoint: float, yPoint: float):
        ...
        angular_speed = max(-max_angular_speed, min(Kp_angular * angle_diff, max_angular_speed))
    ...
    def step(self):
        ...
        ballPos = self.ball_translation_field.getSFVec3f()
        puckPos = self.puck_translation_field.getSFVec3f()
        goalPos = [self.MiddleOfGoal[0], self.MiddleOfGoal[1], ballPos[2]]
        ...
        # Distances for logic
        dist_behind = math.hypot(behind_ball_point[0] - puckPos[0], behind_ball_point[1] - puckPos[1])
